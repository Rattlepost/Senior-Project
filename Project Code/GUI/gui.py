...
class LeftFrame(
    ctk.CTkFrame
):  # contains the bottons to switch between the different app pages
    ...
    def create_widgets(self, right_frame):  # creates the buttons for the left frame
        self.button_dungeon = ctk.CTkButton(
            self,
            text="Dungeon Generator",
            command=lambda: right_frame.swap_frame("dungeon"),
        )
        self.button_tracker = ctk.CTkButton(
            self,
            text="Character Tracker",
            command=lambda: right_frame.swap_frame("tracker"),
        )
        self.button_npc = ctk.CTkButton(
            self, text="NPC Generator", command=lambda: right_frame.swap_frame("npc")
        )
...
class RightFrame(
    ctk.CTkFrame
):  # contains the main content of the app, the different pages
    def __init__(self, master):
        ...
        self.dungeon = DunFr.DungeonFrame(
            self, num_rooms=5
    ...
    def swap_frame(
        self, frame
    ):  # function to switch between the different pages of the app
