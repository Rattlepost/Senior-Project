...
class DungeonFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):  # Create buttons and labels
        ...
        self.generate_button = ctk.CTkButton(
            self, text="Generate Dungeon", command=self.start_generation
        )
        ...
        self.popout_button = ctk.CTkButton(
            self, text="Pop Out Descriptions", command=self.popout_description_box
        )
    ...
    def start_generation(self):  # Start dungeon generation with loading text and thread the generation to avoid freezing the GUI  
        self.generate_button.configure(state="disabled") # Disable the generate button to avoid errors
        self.popout_button.configure(state="disabled") # Disable the popout button to avoid errors
    ...
    def draw_dungeon(self, dungeon):  # Draw the dungeon on the canvas
        ...
        for room in dungeon:
            ...
            if room.shape == "circle":
                ...
                self.canvas.create_rectangle(
                    x - 20, y - 20, x + 20, y + 20, fill="white"
                )
        ...
        for room in dungeon:
            ...
            self.canvas.create_text(
                x, y, text=str(room.room_id), fill="black", font=("Arial", 12)
            )
    ...
    def generate_room_descriptions(
        self, dungeon
    ):  # Generate room descriptions using AI
        descriptions = []
        for room in dungeon:
            prompt = f"""
            ...
            try:
                ...
                descriptions.append(
                    f"Room {room.room_id}: Failed to generate description. Error: {e}"
                )
