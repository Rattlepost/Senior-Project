...
class DiceRoller(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.result_box = ctk.CTkTextbox(self, width=300, height=200, font=("Arial", 16))
        ...
        try:
            ...
            if "d" in input_text:
                ...
                if num > 0 and sides > 0:
                    ...
                    self.send_to_result_box("Invalid: Enter positive numbers (e.g., 2d4)")
            elif "" in input_text:
                return
            else:
    ...
    def roll_dice(self, sides, num):
        ...
        self.send_to_result_box(f"Rolled {num}d{sides}: {result} (Rolls: {', '.join(map(str, rolls))})")
    ...
    def fill_previous_entry(self, event):
        if self.history and self.history_index > 0:
            ...
            self.entry.insert(0, self.history[self.history_index])  # Fill with the previous entry