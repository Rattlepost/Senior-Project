...
class TrackerFrame(ctk.CTkFrame):
    ...
    def create_widgets(self):  # Create buttons and labels
        ...
        self.add_frame_button = ctk.CTkButton(
            self, text="Add Character", command=self.add_new_npc
        )
        self.clear_roll_button = ctk.CTkButton(
            self, text="Clear Roll", command=self.clear_roll
        )
        self.roll_all_button = ctk.CTkButton(
            self, text="Roll All", command=self.roll_all
        )
        self.clear_npcs_button = ctk.CTkButton(
            self, text="Clear NPCs", command=self.clear_npcs
        )
    ...
    def create_new_frame(
        self, npc_data
    ):  # Creates a new NPC frame and adds it to the container
        new_frame = NPC.CharacterFrame(
            self.frame_container, npc_data, self.save_npcs
        ) 
    ...
    def save_npcs(self):  # Save all NPC data to a file
        ...
        for child in self.frame_container.winfo_children():
            if isinstance(child, NPC.CharacterFrame):
                npc_list.append(
                    child.get_npc_data()
                )  # Collect NPC data from each frame
